...
for i in range(len(landuse_train)):
    ...
    training = sentinel2_patches.reduceRegions(collection=landuse_train[i], \
                                                scale=10,\
                                                reducer='first', \
                                                tileScale=1)
    ...
    if (i == 0):
        ...
        task = ee.batch.Export.table.toDrive(collection=training.limit(2), \
                                             selectors=['public_id']+bands+label_names, \
                                             description='00_train_patches_sample', \
                                             fileFormat='CSV', \
                                             folder='EE_Data')
        task.start()
    ...
    task = ee.batch.Export.table.toDrive(collection=training, \
                                    selectors=['public_id']+bands+label_names, \
                                    description='train_patches_'+"{0:04}".format(i),\
                                    fileFormat='TFRecord', \
                                    folder='EE_Data_training_v02')
    task.start()


for i in range(len(landuse_val)):
    sentinel2_patches = sentinel2_composite.neighborhoodToArray(ee.Kernel.square(3))
    validation = sentinel2_patches.reduceRegions(collection=landuse_val[i], \
                                    scale=10, \
                                    reducer='first', \
                                    tileScale=1)
    task = ee.batch.Export.table.toDrive(collection=validation,
                                    selectors=['public_id']+bands+label_names, \
                                    description='validate_patches_'+"{0:04}".format(i), \
                                    fileFormat='TFRecord', \
                                    folder='EE_Data_validation_v02')
    task.start()
